...
class DualPathSRPipeline:
    ...
    def __init__(
        ...
        Args:
            ...
            use_tta: Whether to run Test-Time Augmentation (4-fold dihedral ensembling)
                to max out sharpness and cancel noise.
        ...
        # 1. Load Sen2SR-RRDB Model (Path A)
        try:
    ...
    def _extract_rgbn(self, tensor_10b: torch.Tensor) -> torch.Tensor:
        """Extract and format 10-band tensor to [B02, B03, B04, B08] (RGBN) for Sen2SR-RRDB.
        ...
        Target ordering for Sen2SR-RRDB:
            0: B02 (Blue)
            1: B03 (Green)
            2: B04 (Red)
            3: B08 (NIR)
    ...
    def run_inference(
        ...
        Returns:
            Dict[str, torch.Tensor]: Dictionary containing:
                - 'sr_diffusion': Raw 4-band output (aliased to sr_able for compatibility)
                - 'sr_able': Raw 4-band Sen2SR-RRDB output (1, 4, 4H, 4W)
        ...
        # -------------------------------------------------------------
        # Path A: Sen2SR-RRDB (RGB+NIR 4x)
        # -------------------------------------------------------------
        lr_rgbn = self._extract_rgbn(lr_gpu)
        run_tta = self.use_tta if use_tta is None else use_tta
        logger.info(
            "[%s] Path A (Sen2SR-RRDB): Running 4-band super-resolution (TTA=%s)...",
            aoi_name,
            run_tta,
        )
        sr_able = None
        try:
            ...
            if "out of memory" in str(exc).lower():
                logger.warning(
                    "[%s] CUDA OOM in Sen2SR-RRDB — retrying on CPU",
                    aoi_name,
                )
                ...
                raise InferenceError(
                    f"[{aoi_name}] Path A (Sen2SR-RRDB) execution failed: {exc}"
                ) from exc
        except Exception as exc:
            raise InferenceError(
                f"[{aoi_name}] Path A (Sen2SR-RRDB) execution failed: {exc}"
        ...
        if sr_able is not None and (
            torch.isnan(sr_able).any() or torch.isinf(sr_able).any()
        ):
        ...
        # -------------------------------------------------------------
        # Map Path A channels: 0:B02, 1:B03, 2:B04, 3:B08
        # Into 10-band tensor: [B02, B03, B04, B05, B06, B07, B08, B8A, B11, B12]
        sr_fused = sr_sen2sr.clone()
        if sr_able is not None:
            sr_fused[:, 0:1] = sr_able[:, 0:1]  # B02 (Blue)
            sr_fused[:, 1:2] = sr_able[:, 1:2]  # B03 (Green)
            sr_fused[:, 2:3] = sr_able[:, 2:3]  # B04 (Red)
            sr_fused[:, 6:7] = sr_able[:, 3:4]  # B08 (NIR)
        ...
        return {
            "sr_diffusion": sr_able,  # Kept for backward compatibility
            "sr_able": sr_able,
            "sr_sen2sr": sr_sen2sr,
            "sr_fused": sr_fused,
            "sr_final": sr_final,
