"""
Stage 1 RGBN Sen2SR-RRDB Training Pipeline (Direct Synthesis, Zero +bicubic shortcut).
Designed for 4 to 4.5 Hour Execution with High-Frequency Edge Supervision & SAM Band Lock.
"""
...
def main():
    ...
    print(f"  Model Architecture : 4-Band Sen2SR-RRDB ({n_params:.2f}M params - Pure Conv & PixelShuffle)")
    print(f"  Residual Shortcut  : None (Direct Synthesis - Zero +bicubic bypass)")

    # 2. Loss & Optimizer
    lc = cfg.get("loss", {})
    criterion = Sen2SRLoss(
        w_l1=lc.get("w_l1", 1.0),
        w_sam=lc.get("w_sam", 0.2),
        w_lap=lc.get("w_lap", 0.4),
        w_grad=lc.get("w_grad", 0.4),
    ...
    sc = tc["scheduler"]
    warmup_ep = sc.get("warmup_epochs", 2)
    ...
    print(f"  Dataset Status     : {len(train_loader.dataset):,} Train Patches | {len(val_loader.dataset):,} Val Patches")
    print(f"  Batch Config       : {tc['batch_size']} (effective: {tc['batch_size']*ga}) | Batches/Epoch: {batches_per_ep}")
    print(f"  Epoch Plan         : {eps} Epochs ({total_target_batches:,} Batches) ~ 4.2 to 4.5 Hours")
    print("=" * 85, flush=True)
    ...
    for epoch in range(start_ep, eps):
        ...
        pbar = tqdm(range(batches_per_ep), desc=f"Epoch {epoch+1:02d}/{eps:02d}", ncols=105, leave=True)
        for step in pbar:
            try:
            ...
            if step % 5 == 0 or step == batches_per_ep - 1:
                ...
                # Write live status for user CMD monitor
                try:
        ...
        print(f"  Epoch {epoch+1:02d}/{eps:02d} | Loss: {avg_loss:.4f} | Val PSNR: {vp:.2f} dB | SSIM: {vs:.4f} | SAM: {v_sam:.4f} | Time: {elapsed:.1f}m{best_flag}", flush=True)

    cf.close()
    total_time_hr = (time.time() - total_start_time) / 3600.0
    print("\n" + "=" * 85)
    print(f"  TRAINING COMPLETED SUCCESSFULLY in {total_time_hr:.2f} Hours!")
    print(f"  Best Validation PSNR: {best_psnr:.2f} dB | Best SSIM: {best_ssim:.4f}")
    print("=" * 85)

    # Automatically generate final visual comparison upon completion
    try:
        cmd = f"python scripts/check_sen2sr_results.py"
        os.system(cmd)
    except Exception:
        pass
